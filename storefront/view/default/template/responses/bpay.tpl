...
<form action="<?php echo str_replace('&', '&amp;', $BPAY_TXN_URL); ?>" method="post" id="checkout">
	<div class="hiddenFormBPAYFieldsDiv">
  	...
  	<?php
  		if(isset($BPAY_PROMOCODE_STATUS) && isset($BPAY_promocode_LOCAL_VALIDATION) && isset($BPAY_PROMOCODE_VALUE)){
  			if($BPAY_PROMOCODE_STATUS=='enabled'){
  				if(($BPAY_promocode_LOCAL_VALIDATION=='enabled' && trim($BPAY_PROMOCODE_VALUE)!='') || $BPAY_promocode_LOCAL_VALIDATION=='disabled'){
  	?>
	  	<div class="form-group">
	  		...
	  		<div class="col-md-8" style="margin-bottom: 5px;">
				<span class="input-group-btn">
	  				<input type="text" id="promoCode" class="form-control pull-left" placeholder="BPAY Promo Code" style="width: 70%;">
	...
	$(document).ready(function(){
		$('.btnPromoCode').click(function(){
			...
			if($.trim($("#promoCode").val())!=''){
				...
				$.ajax({
					url: 'index.php?rt=extension/bpay/applyCode',
					type: 'post',
					dataType: 'json',
					...
					success: function(res){
						...
						if(res.hiddenFields.length > 0){
							$('.hiddenFormBPAYFieldsDiv').html(res.hiddenFields);		
						}
					}
				});
